...
# Final output: ./output/[sic3]/[sic3]_syndata_stepkng1_[kernel].csv
...
generate_syn_emp = function(raw_data, syn_data, syn_var, taus, h, epsilon, Cx, kernel, mod) {
    ...
    tmp = stepwiseKNG2(data = raw_data, total_eps = epsilon, tau = taus, scale = scale, #1e-1,
...
syn_data_all_years = function(seed) {
    start_year = ifelse(seed == 4, 1985, ifelse(seed <=2, 1983, ifelse(seed == 5, 1989, 1987)))
    for (curr_year in start_year:2000) { # rmb to change back to 2021
        ...
        if (curr_year == 1977) { # remember to change to _seed, seed
            ...
            syn_data = read.csv(paste0("./output/", sic3, "/", sic3, "_syndata_stepkng1_", kernel, "_", curr_year-1, "_seed", seed, ".csv")) # change method
        }
            ...
            if (nrow (raw_data_birth) > 0 & nrow(syn_data_birth) > 0) {
                ...
                while(is.null(ans_birth)) {
                    ...
                    ans_birth = tryCatch(withTimeout({generate_syn_emp(raw_data = raw_data_birth, 
                                                                       ...
                                                                       kernel = kernel, mod = 'birth')}, timeout= 1800), 
            ...
            if(syn_var == 'emp_1977') {
                ...
                while(is.null(ans_cont)) {
                    ...
                    ans_cont = tryCatch(withTimeout({generate_syn_emp(raw_data = raw_data_cont, 
                                                                      ...
                                                                      taus = taus, h = h, epsilon = epsilon, Cx = max(Cx_tmp, 25), 
                                                                      kernel = kernel, mod = 'continuer')}, timeout= 1800), 
                                        error = function(cond) NULL)
                    toc()
                ...
                if (nrow (raw_data_cont_tmp1) > 0 & nrow(syn_data_cont_tmp1) > 0) {
                    ...
                    while(is.null(ans_cont_tmp1)) {
                        ...
                        ans_cont_tmp1 = tryCatch(withTimeout({generate_syn_emp(raw_data = raw_data_cont_tmp1, 
                                                                               ...
                                                                               taus = taus, h = h, epsilon = epsilon, Cx = max(Cx_tmp, 25), 
                                                                               kernel = kernel, mod = 'continuer')}, timeout= 2700), 
                                                 error = function(cond) NULL)
                        toc()
        ...
        if (curr_year == 2000) {
            output_file = paste0("./output/", sic3, "/", sic3, "_syndata_stepkng1_", kernel, "_seed", seed, ".csv")
        } else {
            output_file = paste0("./output/", sic3, "/", sic3, "_syndata_stepkng1_", kernel, "_", curr_year, "_seed", seed, ".csv")
        }
        
        write.csv(syn_data, output_file, row.names = FALSE)
        
        old_output_file = paste0("./output/", sic3, "/", sic3, "_syndata_stepkng1_", kernel, "_", curr_year-1, "_seed", seed, ".csv")
...
# print(num_cores)
# workers=makeCluster(num_cores, output_file = paste0('05b_generate_syn_emp_stepkng1_', sic3, '.Rout'))
