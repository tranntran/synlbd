    ...
    while (retry) {
        ...
        for (i in 2:nbatch){
            ...
            if (i %% 1000 == 0) {
                ...
                cat(scale_ans[[1]][i], ' ', scale_ans[[2]][i], ' ', scale_ans[[3]][i], '\n')
                if (any(acc < 1e-5)) {
                    idx = which(acc <1e-5)
                    for (id in idx) {
                        print(batch[[id]][1, ])
        ...
        for (j in 1:nchains) {
            ...
            if (ct[[j]] == update_after){
                if (method == "fixed" | dim == 1) {
                    ...
                    sigma = cor(na.omit(batch[[j]])) + diag(dim)*0.00002
...
sandwichKNG = function(data, total_eps, median_eps = NULL, main_tau_eps = NULL,
    ...
    if (length(sandwich_tau_lower) > 0){
        ...
        for (i in 1:length(sandwich_tau_lower)){
            ...
            #main_tau_eps
            out = constrKNGSandwich(init = list(upperbeta, lowerbeta, (upperbeta + lowerbeta)/2), 
