...
# Final output: ./output/[sic3]/[sic3]_syndata_stepkng_[kernel].csv
...
syn_data_all_years = function(seed) {
    for (curr_year in 1977:2000) { # rmb to change back to 2021
        ...
        if (curr_year == 1977) { # remember to change to _seed, seed
            ...
            syn_data = read.csv(paste0("./output/", sic3, "/", sic3, "_syndata_stepkng_", kernel, "_", curr_year-1, "_seed", seed, ".csv")) # change method
        }
            ...
            if (nrow (raw_data_birth) > 0 & nrow(syn_data_birth) > 0) {
                ...
                while(is.null(ans_birth)) {
                    ...
                    ans_birth = tryCatch(withTimeout({generate_syn_emp(raw_data = raw_data_birth, 
                                                                           ...
                                                                           kernel = kernel, mod = 'birth')}, timeout= 900), 
            ...
            if(syn_var == 'emp_1977') {
                ...
                while(is.null(ans_cont)) {
                    ...
                    ans_cont = tryCatch(withTimeout({generate_syn_emp(raw_data = raw_data_cont, 
                                                                          ...
                                                                          kernel = kernel, mod = 'continuer')}, timeout= 1500), 
                                            error = function(cond) NULL)
                    toc()
                ...
                if (nrow (raw_data_cont_tmp0) > 0 & nrow(syn_data_cont_tmp0) > 0) {
                    ...
                    while(is.null(ans_cont_tmp0)) {
                        ...
                        ans_cont_tmp0 = tryCatch(withTimeout({generate_syn_emp(raw_data = raw_data_cont_tmp0, 
                                                                          ...
                                                                          taus = taus, h = h, epsilon = epsilon, Cx = max(Cx_tmp, 25), 
                                                                          kernel = kernel, mod = 'continuer')}, timeout= 2000), 
                                            error = function(cond) NULL)
                        toc()
        ...
        if (curr_year == 2000) {
            output_file = paste0("./output/", sic3, "/", sic3, "_syndata_stepkng_", kernel, "_seed", seed, ".csv")
        } else {
            output_file = paste0("./output/", sic3, "/", sic3, "_syndata_stepkng_", kernel, "_", curr_year, "_seed", seed, ".csv")
        }
        
        write.csv(syn_data, output_file, row.names = FALSE)
        
        old_output_file = paste0("./output/", sic3, "/", sic3, "_syndata_stepkng_", kernel, "_", curr_year-1, "_seed", seed, ".csv")
...
sic3 = 178 #as.numeric(commandArgs(trailingOnly = TRUE)) #2371
sic2 = floor(sic3/10)
...
# print(num_cores)
# workers=makeCluster(num_cores, output_file = paste0('05b_generate_syn_emp_stepkng_', sic3, '.Rout'))
