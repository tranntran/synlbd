...
sic3 = 239 #178 #as.numeric(commandArgs(trailingOnly = TRUE))

var_list = c('firstyear', 'lastyear', paste('mu5', seq(1977, 2002, 5), sep = '_'), 
             paste('emp', c(1977:2000), sep = '_'))

# tic()
seed = as.integer(commandArgs(trailingOnly = TRUE))
# for(seed in 1:5) {
    # load libraries
library(dplyr)
library(synthpop)
...
tab$sic3[1] = sic3
sic2 = floor(sic3/10)
syn_data = read.csv(paste0("./output/", sic3, "/", sic3, "_syndata_stepkng1_logistic_seed", seed, ".csv"))
...
output = paste0("./output/general_utility/pMSE_utility_syndata_stepkng1.csv")
