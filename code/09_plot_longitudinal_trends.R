...
png(paste0('./output/plots/', sic, '_jcr.png'), width = 750, height = 500)
ggplot(data = plot_final, aes(x = year, group = methods)) + 
  ...
  labs(title=paste0("Job Creation Rate by Year for Industry ", sic), 
...
png(paste0('./output/plots/', sic, '_njcr.png'), width = 750, height = 500)
ggplot(data = plot_final, aes(x = year, group = methods)) + 
  geom_line(aes(y = mean, color = methods, size = methods)) + 
  geom_ribbon(aes(y = mean, ymin = min_val, ymax = max_val, fill = methods), alpha = .3) +
  labs(title=paste0("Net Job Creation Rate by Year ", sic), 
